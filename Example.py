...
bounds = array([[-0.2,0.2], [-0.2, 0.2],[-0.3,0.3]])  # input limits [(x_min,x_max)]
number_particles = int(30)  # amount of particles
interactions = int(5000)  # amount of interactions that each particle will make
...
graph = Graph( pso.history.position[1:, :],pso.history.fitness[1:,:], pso.history.velocity[:, 0], pso.history.vel_average[:, 0])
graph.bokeh()
graph.plotly()
